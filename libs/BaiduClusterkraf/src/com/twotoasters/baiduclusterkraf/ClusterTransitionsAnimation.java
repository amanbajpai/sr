...
class ClusterTransitionsAnimation implements AnimatorListener, AnimatorUpdateListener {
    ...
    ClusterTransitionsAnimation(BaiduMap map, Options options, Host host) {
        mapRef = new WeakReference<BaiduMap>(map);
    ...
    private Marker addMarker(BaiduMap map, MarkerOptionsChooser moc, ClusterPoint clusterPoint) {
        MarkerOptions mo = new MarkerOptions();
        mo.position(clusterPoint.getMapPosition());
        if (moc != null) {
            moc.choose(mo, clusterPoint);
        }
        return (Marker) (map.addOverlay(mo));
    }
